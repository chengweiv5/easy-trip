...
- `confirmationSyncFailed=true` 时拒绝任何滞后或直接派发的 `ConfirmDelete`；恢复只能走 `RetryDeletionSync`，确保 delete service exactly-once。
- 聚焦恢复由 `2e698f7`、`7597706`、`b89a80a` 收口；最终验收 HEAD 为 `b89a80a`。`SyncFailure`/“重新同步”路径只恢复 Flow 订阅，恢复成功不重复 delete。
- 最终自动化事实限定为 JVM 426/426、最新串行 focused combined 12/12、Catalog 47/47、Full UI 47/47、lint/APKs PASS；未据此宣称全量 connected 门禁通过，物理设备验收仍为 DEFERRED。
