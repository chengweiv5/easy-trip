...
| 9 | 固定控制区 | 地图在顶部；顶栏和搜索框均为 48dp；搜索、地点池/单日/全程与抽屉间保留 10dp，抽屉收起时控制区仍可见 | 通过 | 第二轮视觉优化真机复验通过 |
| 10 | 抽屉手势 | 不显示收起/半屏/展开按钮；通过短横线向上滑展开、向下滑收起，三态均可达 | 通过 | 收起稳定不回弹；恢复短横线位于搜索框上方，真机复验通过 |
