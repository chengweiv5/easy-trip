...
class ThemeRealMapVerificationTest {
    @get:Rule val compose = createAndroidComposeRule<ComponentActivity>()

    @Test fun fiveThemesKeepRealMapTilesCameraAndDayColors() {
        ...
        val overlay = mutableStateOf<WorkspaceOverlay>(WorkspaceOverlay.None)
        val preference = java.io.File(compose.activity.cacheDir, "real-map-theme-${System.nanoTime()}")
        val store = ThemePreferenceStore(AtomicThemePersistence(preference))
        var creations = 0
        var destroys = 0
        lateinit var nativeMap: MapView
        var applied = ThemePalette.LAKE
        ...
        compose.setContent { EasyTripThemeHost(store) {
            ...
            val markers = points.indices.map { i -> MapMarkerUi("place-$i", points[i], names[i], emptyList(),
                ...
                badgeText = (i + 1).toString(), savedPlaceId = "place-$i", scheduled = isItinerary,
            ...
            val model = MapUiModel(markers = markers,
                polylines = if (isItinerary) listOf(MapPolylineUi("route", "day-1", points, routeColorForDay(0))) else emptyList())
            TripWorkspaceContent(
                pageState = TripWorkspacePageState.Ready(TripWorkspaceUiState(
                    ...
                    sheetLevel = WorkspaceSheetLevel.HALF, map = model, overlay = overlay.value,
                ).toReadyState()),
                mapState = WorkspaceMapState.Ready, onAction = { action ->
                    when (action) {
                        is TripWorkspaceAction.OpenOverlay -> overlay.value = action.overlay
                        TripWorkspaceAction.CloseOverlay -> overlay.value = WorkspaceOverlay.None
                        else -> Unit
                    }
                },
                ...
                mapContent = { layout -> AmapComposeMap(
                    ...
                    hostFactory = { context ->
                        ...
                        nativeMap = actual.view as MapView
                        creations++
                        object : AmapMapHost by actual {
                            override fun onDestroy() { destroys++; actual.onDestroy() }
                            override fun setPalette(palette: ThemePalette) { applied = palette; actual.setPalette(palette) }
        ...
        compose.runOnIdle { camera = nativeMap.map.cameraPosition }
        for (theme in ThemePalette.entries) {
            if (theme != store.theme.value) {
                ...
                compose.onNodeWithTag("theme-option-${theme.id}").performScrollTo().performClick()
                compose.onNodeWithTag("theme-apply").performClick()
                compose.waitUntil(5_000) { store.theme.value == theme && applied == theme }
            }
            compose.runOnIdle {
                val after = nativeMap.map.cameraPosition
                assertEquals(camera!!.target.latitude, after.target.latitude, .000001)
                assertEquals(camera!!.target.longitude, after.target.longitude, .000001)
                assertEquals(camera!!.zoom, after.zoom, .0001f)
                assertEquals(1, creations)
                assertEquals(0, destroys)
            }
            android.os.SystemClock.sleep(750)
            save("pool-${theme.id}.png")
        }
    ...
    private fun save(name: String) {
        ...
        java.io.File(java.io.File(compose.activity.getExternalFilesDir(null), "v160-real-map").apply { mkdirs() }, name).outputStream().use { bitmap.compress(Bitmap.CompressFormat.PNG, 100, it) }
    }
}
