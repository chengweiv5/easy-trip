...
- 模拟器 emulator-5596，easy_trip_p60pro 的只读临时实例，未操作物理设备；地图使用测试 host 验证生命周期、配色传递与相机请求不重复，原生标记独立像素检查。用户随后授权本地配置复制，真实高德地图补验已完成，见 ../v1.6.0-real-map/verification.md。

## 环境说明

初次复制因自动审批拒绝而取消；2026-09-23 用户明确回复「授权」后，已将主仓库 local.properties 复制到当前工作区（0600、Git ignored、未追踪），确认调试包注入正确且真实地图可正常加载。未输出密钥、未提交本地配置、未发布 APK。
