...
收藏地点 · 按天安排行程 · 在地图上看清每一站

一款本地优先的 Android 旅行规划应用，适合周末出游、城市漫游和多日自由行。
...
> 当前正式版为 **v1.1.0**，可在发布页下载 APK。`main` 已更新到 **v1.2.0**，尚未发布对应正式 APK；下方注明了截图版本。
...
| 临时想换顺序，或给某一站多留点时间 | 拖动调整地点顺序，移到其他日期；滑动选择到达时间，以小时设置停留时长。 |
...
### v1.2.0 源码预览：重要的备注，抬眼就能看见

门票预约、入园提醒、酒店入住安排，都可以留在对应地点旁。行程与地点池默认显示两行备注，长内容可原位展开、收起；全程合并连续重复地点时，也会保留各次到访的备注。

> 以下是 **v1.2.0 已实现但尚未正式发布**的备注速览，当前 v1.1.0 正式 APK 尚不包含这一展示方式。
