...
class CollectionTogglePolicyTest {
    ...
    @Test fun `unused saved place removes immediately`() {
        assertEquals(CollectionDecision.RemoveNow("place-1"), decideCollectionToggle(candidate, saved, PlaceDeletionImpact(0, 0)))
    }

    @Test fun `used saved place requires impact confirmation`() {
        val impact = PlaceDeletionImpact(4, 3)
        assertEquals(CollectionDecision.Confirm("place-1", impact), decideCollectionToggle(candidate, saved, impact))
    }
    ...
    @Test fun `saved place rejects negative usage`() {
        assertThrows(IllegalArgumentException::class.java) {
            decideCollectionToggle(candidate, saved, PlaceDeletionImpact(-1, 0))
        }
    }
}
