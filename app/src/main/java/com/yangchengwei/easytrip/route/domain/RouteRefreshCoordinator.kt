...
interface RouteRefreshCoordinator { fun start(scope:CoroutineScope); suspend fun retry(legId:String):Boolean; suspend fun overrideMode(legId:String,mode:TransportMode):Boolean }
class DefaultRouteRefreshCoordinator(private val repository:RouteLegRepository,private val planner:RoutePlanner,private val networkMonitor:NetworkMonitor,private val onRequeueError:(Throwable)->Unit={}):RouteRefreshCoordinator {
 ...
 override suspend fun overrideMode(legId:String,mode:TransportMode)=repository.overrideMode(legId,mode,networkMonitor.isOnline.value)
