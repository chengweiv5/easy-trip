...
interface RouteRefreshCoordinator { fun start(scope:CoroutineScope); suspend fun retry(legId:String):Boolean; suspend fun retry(legId:String,expectedVersion:Long):Boolean = false; suspend fun updateDetails(legId:String,selectedModeOverride:TransportMode?,durationOverrideSeconds:Int?,note:String?):Boolean }
...
override suspend fun retry(legId:String)=repository.retry(legId,networkMonitor.isOnline.value)
	override suspend fun retry(legId:String,expectedVersion:Long)=repository.retry(legId,expectedVersion,networkMonitor.isOnline.value)
