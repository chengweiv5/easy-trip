...
internal fun placeCityGroups(rows: List<SavedPlaceRowUi>): List<PlaceCityGroup> {
    ...
    return rows.groupBy { it.place.city()?.key ?: UNKNOWN_CITY_KEY }.map { (key, places) ->
        PlaceCityGroup(key, places.first().place.city()?.name ?: "城市待补全", places)
    }.sortedWith { a, b ->
        when {
...
enum class PlaceScheduleFilter(val label: String) { ALL("全部"), UNSCHEDULED("未排入"), SCHEDULED("已排入") }

internal fun filterPlaceCityGroups(rows: List<SavedPlaceRowUi>, cityKey: String?, query: String = "", schedule: PlaceScheduleFilter = PlaceScheduleFilter.ALL): List<PlaceCityGroup> {
    val keyword = query.trim()
    return placeCityGroups(rows).filter { cityKey == null || it.key == cityKey }.mapNotNull { group ->
        val matches = group.rows.filter {
            (schedule == PlaceScheduleFilter.ALL || (it.itineraryOccurrenceCount > 0) == (schedule == PlaceScheduleFilter.SCHEDULED)) &&
                (keyword.isEmpty() || it.name.contains(keyword, true) || it.address.contains(keyword, true) || group.name.contains(keyword, true))
        }
