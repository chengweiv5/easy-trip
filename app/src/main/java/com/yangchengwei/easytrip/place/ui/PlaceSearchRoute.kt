...
fun PlaceSearchRoute(
    ...
    state.pendingCollectionRemoval?.let { pending ->
        AlertDialog(
            onDismissRequest = { viewModel.dispatch(PlaceSearchAction.DismissRemovalConfirmation) },
            title = { Text("取消收藏 ${pending.place.name}？") },
            text = { Text("将同时删除 ${pending.usageCount} 次行程安排及受影响路线。") },
            confirmButton = {
                TextButton(onClick = { viewModel.dispatch(PlaceSearchAction.ConfirmRemoval) }) {
                    Text("确认取消收藏")
                }
            },
            dismissButton = {
                TextButton(onClick = { viewModel.dispatch(PlaceSearchAction.DismissRemovalConfirmation) }) {
                    Text("取消")
                }
            },
        )
